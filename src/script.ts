...
const passField = <HTMLInputElement>document.getElementById("password-field");
...
const strengthIndicator: any = document.getElementById("strength-indicator");
const lowercaseString: string = "abcdefghijklmnopqrstuvwxyz";
const uppercaseString: string = "abcdefghijklmnopqrstuvwxyz".toUpperCase();
const specialString: string = "!@#$%&*....";
const numbersString: string = "123456789";
let password: any = "";
...
function generate() {
  ...
  if (uppercase.checked) {
    finalPassword +=
      uppercaseString.split("")[
        Math.floor(Math.random() * uppercaseString.length)
      ];
  ...
  if (lowercase.checked) {
    finalPassword +=
      lowercaseString.split("")[
        Math.floor(Math.random() * lowercaseString.length)
      ];
    finalPassword = finalPassword.slice(1);
    console.log(finalPassword);
  }
